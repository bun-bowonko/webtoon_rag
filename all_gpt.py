...
if __name__ == "__main__":
    ...
    for data_idx in tqdm(range(args.start_chapter_num, args.end_chapter_num+1)):
        ...
        for i, r in enumerate(response_split):
            ...
            #한 문장씩 리뷰
            # <translate> </translate> 형태로 못 만들 경우 무한 반복
            review_result = None
            while not review_result:
                ...
                print(review)
                review_result = extract_translation(review)
            review_li.append(review_result)
        #기존 번역 개수 == 리뷰 번역 개수 == 한글 대화 개수
        assert len(response_split) == len(review_li)

        hangle = []
        first_translate = []
        second_translate = []
        for i in range(len(response_split)):
            hangle.append(clean_text(source[i]))
            first_translate.append(clean_text2(response_split[i]))
            second_translate.append(review_li[i])

        label = answers[data_idx]
        with open(f"{args.save_path}/result_{data_idx}.csv", mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(['hangle', 'first_translate', 'second_translate', 'label'])
            for row in zip(hangle, first_translate, second_translate, label):
                writer.writerow(row)
